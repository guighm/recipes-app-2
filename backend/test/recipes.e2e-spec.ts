...
describe('Recipes (e2e)', () => {
  ...
  const recipePayload = {
    ...
    servings: 8,
    difficulty: 'easy',
    imageUrl: 'http://img.com/bolo.jpg',
  };
  ...
  it('adds ingredients and steps and lists them nested under the recipe', async () => {
    ...
    await request(server)
      .post('/ingredients')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ recipeId, name: 'Farinha', quantity: '1kg' })
      .expect(201);

    await request(server)
      .post('/steps')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ recipeId, stepNumber: 1, description: 'Misture tudo' })
      .expect(201);
    ...
    await request(server)
      ...
      .expect((res) => {
        expect(res.body).toEqual([
          { id: expect.any(Number), name: 'Farinha', quantity: '1kg' },
        ]);
      });
